...
;
    ...
    listening.socket.poll:      mov  ebx, socket.data.event.socket.descriptor                           ; ebx = Pointer to descriptor associated with listening socket.
                                ...
                                jmp  socket.accept.error                                                ; Nope. Something is seriously wrong here. Go tell the admin we're screwed and bail out.
...
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    greet.loop                  cmp  dword [esi+connection.descriptor.index], dead.socket               ; Is the current entry occupied?
                                ...
                                push eax                                                                ; Preserve descriptor associated with new connection. If we're here, the table is full up and there's no room for a new person. Bad news, bud.
