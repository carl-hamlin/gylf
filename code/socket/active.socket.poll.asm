...
;   Returns:        None.
    ...
    active.socket.poll:           mov     esi, connection.table                                ; esi = pointer to table of potentially active connections.
                                  ...
                                  mov     dword [sockets.left], ecx                            ; Store number of connections to check for activity in memory so we aren't constrained in our use of the ecx register later on.
...
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    poll.active.socket            mov     dword [sockets.left], ecx                            ; Store number of sockets left to check in memory - we're going to need the ecx register for something else momentarily.
                                  ...
                                  mov     ecx, dword [sockets.left]                            ; No. Let's get the number of sockets left back into the ecx register so the loop will iterate properly.
