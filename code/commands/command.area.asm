...
command.area.bad:			pop	esi										; Restore index to connection entry.
					...
					call	write.descriptor								; Tell the admin there's someone online with admin priviledges
...
command.area.find.first.argument:	cmp	byte [esi], 020h								; Did the user put a space between the command and the first
																; argument?
...
command.area.loop.1:			cmp	byte [esi], "|"									; Have we hit the argument demarcator?
					...
					movsb											; Move a byte of the first argument over to the name field in the
																; area header.
					inc	edx										; edx - Count of how many bytes we've moved so far.

					loop	command.area.loop.1								; Go move the rest of the first argument.

					jmp	command.area.bad								; Looks like the argument is longer than the field to which it is
																; to be stored. Go educate the user on the fine points of syntax.
...
command.area.loop.2			cmp	byte [esi], 00h									; Have we hit the end of the command string?
					jz	command.area.description.l							; Yes. Go process the description and get started finding a
																; filename to which to store this data.
...
command.area.filename.loop:		mov	ebx, area.filename								; ebx - Pointer to filename entry for area filenames.
					...
					or	eax, eax									; If eax is signed, the file doesn't exist, which means we've
																; found a candidate.
...
command.area.next.file:			mov	ebx, eax									; ebx - Descriptor associated with an unusable area file.
					...
					jz	command.area.filename.00000010							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.00000010:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.00000100							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.00000100:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.00001000							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.00001000:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.00010000							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.00010000:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.00100000							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.00100000:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.01000000							; Yes. Go zero it out and address the next place up.

					jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and
																; check the resulting filename.

command.area.filename.01000000:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.
...
command.area.filename.10000000:		mov	byte [ebx], char.0								; Zero out the byte at [ebx].
					dec	ebx										; ebx - Pointer to next place up.
					inc	eax										; eax - Count of places.

					cmp	byte [ebx], char.9								; Is the byte at [ebx] a "9"?
					jz	command.area.filename.out.of.areas						; Go tell the admin and user that we're plum out of area files.
