...
;
;			check.admin				code/functions/check.admin.asm
;			command.area.admin			code/commands/command.area.asm
;			local.data.area.bad.syntax.message	raw.data/messages/messages.local.dat
;			local.data.area.bad.syntax.message.l	raw.data/messages/messages.local.dat
;			connection.descriptor.index		definitions/socket.def
;			socket.data.send.socket.descriptor	raw.data/socket/socket.dat
;			socket.data.send.buffer.pointer		raw.data/socket/socket.dat
;			socket.data.area.bad.syntax.message	raw.data/messages/messages.out.dat
;			socket.data.send.buffer.l		raw.data/socket/socket.dat
;			socket.data.area.bad.syntax.message.l	raw.data/messages/messages.out.dat
;			socket.send				code/socket/socket.send.asm
;			buffer.1				definitions/buffers.def
;			command.area.l				raw.data/command.data/command.table.dat
;			command.area.start			code/commands/command.area.asm
;			command.area.bad			code/commands/command.area.asm
;			command.area.find.first.argument	code/commands/command.area.asm
;			area.name				definitions/area.def
;			area.name.field.l			definitions/area.def
;			command.area.name.l			code/commands/command.area.asm
;			command.area.loop.1			code/commands/command.area.asm
;			area.name.l				definitions/area.def
;			area.description			definitions/area.def
;			area.description.field.l		definitions/area.def
;			command.area.loop.2			code/commands/command.area.asm
;			command.area.find.second.argument	code/commands/command.area.asm
;			available.areas				definitions/area.def
;			area.description.l			definitions/area.def
;			area.filename				raw.data/file.paths/file.paths.dat
;			open.descriptor				code/functions/open.descriptor.asm
;			command.area.next.file			code/commands/command.area.asm
;			create.file				code/functions/create.file.asm
;			file.indicator				raw.data/function.data/file.indicator.dat
;			write.descriptor			code/functions/write.descriptor.asm
;			dword.l					definitions/miscellaneous.lengths.def
;			close.descriptor			code/functions/close.descriptor.asm
;			local.data.area.created.message		raw.data/messages/messages.local.dat
;			local.data.area.created.message.l	raw.data/messages/messages.local.dat
;			local.data.carriage.return		raw.data/messages/messages.local.dat
;			local.data.carriage.return.l		raw.data/messages/messages.local.dat
;			socket.data.area.created.message	raw.data/messages/messages.out.dat
;			socket.data.area.created.message.l	raw.data/messages/messages.out.dat
;			char.0					definitions/numerals.def
;			char.9					definitions/numerals.def
;			command.area.filename.finder		code/commands/command.area.asm
;			command.area.filename.out.of.areas	code/commands/command.area.asm
;			command.area.prep.next			code/commands/command.area.asm
;			command.area.filename.loop		code/commands/command.area.asm
;			area.num.contained.items		definitions/area.def
;
...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	command.area.loop.1:			cmp	byte [esi], "|"									; Have we hit the argument demarcator?
						...
						movsb											; Move a byte of the first argument over to the name field in the area
																	; header.
						inc	edx										; edx - Count of how many bytes we've moved so far.

						loop	command.area.loop.1								; Go move the rest of the first argument.

						jmp	command.area.bad								; Looks like the argument is longer than the field to which it is to be
																	; stored. Go educate the user on the fine points of syntax.
...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	command.area.loop.2:			cmp	byte [esi], 00h									; Have we hit the end of the command string?
						jz	command.area.description.l							; Yes. Go process the description and get started finding a filename to
																	; which to store this data.

						movsb											; Move a byte of the first argument over to the description field in the
																	; area header.
						inc	edx										; edx - Count of how many bytes we've moved so far.
						...
						jmp	command.area.bad								; Looks like the argument is longer than the field to which it is to be
																	; stored. Go educate the user on the finer points of syntax.
...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	command.area.next.file:			mov	ebx, eax									; ebx - Descriptor associated with an unusable area file.
						...
						jmp	command.area.prep.next								; Increment the byte at [ebx], restore ebx to the last place, and check the
																	; resulting filename.

;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

	command.area.filename.finder:		cmp	eax, 08h
						jz	command.area.filename.out.of.areas
