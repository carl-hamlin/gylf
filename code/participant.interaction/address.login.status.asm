...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	not.logged.in:		mov	edi, esi								; edi = Pointer to connection table.
				...
				or	eax, eax								; If such a file exists, then we'll have succeeded in associating a descriptor with it, and the descriptor will be in eax. If not, then eax will be negative, and contain a sign. We can use the sign to determine which. If eax is signed, we'll treat this like a bad login. If it isn't, then we'll treat it as an good one.
