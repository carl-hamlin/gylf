...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    not.logged.in:        mov   edi, esi                                                                ; edi = Pointer to connection table.
                          ...
                          call  open.descriptor                                                         ; See if there's already a user credentials file named after the data received. If so, associate a descriptor with it.

                          or    eax, eax                                                                ; If such a file exists, then we'll have succeeded in associating a descriptor with it, and the descriptor will be in eax. If not, then eax will be negative, and contain a sign. We can use the sign to determine
                                                                                                        ; which. If eax is signed, we'll treat this like a bad login. If it isn't, then we'll treat it as an good one.
                          js    bad.login                                                               ; Couldn't generate a descriptor for the file. Bad login.

                          call  create.lock.file                                                        ; Assume the character is about to be logged and create a lock file to prevent multiple instances.
                          ...
                          jz    new.login                                                               ; If the received data matches the new character data prototype, go populate the character file.
