...
address.active.status:				call	recv.from.socket							; Get the data from the socket.
						...
						mov	ecx, number.of.commands							; ecx - Number of commands in command table. This number acts as
																; our loop counter.
...
address.active.status.read.command.table:	mov	cl, byte [esi]								; cl - length of command in table.
						inc	esi									; esi - Pointer to string data corresponding to command currently
																; being referenced in the table.

						cmp	cl, al									; Is the length of the received data at least the same as the
																; command in the table?
						ja	address.active.status.next.command					; No. It's not necessary to perform any further comparison; the
																; received data cannot possibly refer to the command currently
																; being read from the table. Move on to the next table entry.

						repz	cmpsb									; Check to see if the received data is the same as the string
																; data corresponding to the command currently being referenced
																; in the table.
						jcxz	address.active.status.found.command					; Did we get all the way through the string before cmpsb cut us
																; loose? If so, go set up the handler and jump to it.

						jmp	address.active.status.next.command.jcxnz				; Otherwise, go set up for the next iteration.

address.active.status.found.command:		pop	edi									; Restore caller's edi register...
...
address.active.status.next.command.jcxnz:	;sub	edx, ecx								; edx - number of bytes received minus the number of bytes run
																; through by cmpsb before a mismatch was discovered.
						mov	edi, ebx								; edi - Pointer to received data.

address.active.status.next.command:		add	esi, ecx								; esi - Pointer to handler for command currently being referenced
																; in the table.
						...
						mov	dword [socket.data.send.buffer.pointer], socket.data.bad.command.error	; Point send function to message indicating that the command was
																; bad.
