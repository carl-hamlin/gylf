...
;   Returns:        None, although it is assumed that if the function returns at all, a listening socket will have been established.
    ...
    set.up.listener:        call create.socket                       ; Create the socket we're going to be using to listen for incoming connections.
                            ...
                            jmp  socket.create.error                 ; Looks like the socket creation process failed somehow. Go display the error message and return control to linux so the admin can figure out what's up.
...
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    socket.bind:            call bind.socket                         ; Bind the socket to a port specified elsewhere.
                            ...
                            jns  socket.listen                       ; that sign to determine if an error occurred. If it didn't, then we'll move on to telling the bound socket to start listening for connections.

                            jmp  socket.bind.error                   ; Looks like the socket binding process failed somehow. Go display the error message and return control to linux so the admin can figure out what's up.
...
;-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    socket.listen:          call listen.socket                       ; Tell the socket to start listening for incoming connections.
                            ...
                            jns  socket.display.success              ; number, with a sign. We can use that sign to determine whether or not an error occurred. If it didn't, we'll tell the admin the socket is ready and hop into the main loop.

                            jmp  socket.listen.error                 ; Looks like the process of telling the socket to listen failed somehow. Go display the error message and return control to linux so the admin can figure out what's up.
