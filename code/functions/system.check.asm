...
system.check:					mov	ecx, local.data.system.check.message				; ecx - Pointer to message indicating system check underweigh
						...
						mov	ecx, local.data.populating.connection.table.message		; ecx - Pointer to message indicating that system check is populating the
															; connection number table.
						mov	edx, local.data.populating.connection.table.message.l		; edx - Length of message.
						call	write.descriptor						; Let the admin know that system.check is populating the connection
															; numbers table.
...
system.check.populate.connection.numbers.loop:	movsd									; Copy the current socket number to the current socket number table entry.
						...
						add	ebx, socket.numbers.l						; ebx - Pointer to byte immediately following placeholder for socket
															; numbers.
...
system.check.populate.connection.numbers.1000:	mov	byte [ebx], char.0						; Zero the byte at [ebx].
						...
						cmp	byte [ebx], char.9						; Is the byte at [ebx] a "9"?
						jz	system.check.populate.connection.numbers.over			; Yes. At this point, we're experiencing a bounds error, so go tell the
															; admin we're fucked and shut down the server.
...
system.check.populate.connection.numbers.over:	mov	ecx, local.data.populate.connection.numbers.over.message	; ecx - Pointer to message indicating we experienced a bounds error and
															; need to shut down the server.
						mov	edx, local.data.populate.connection.numbers.over.message.l	; Length of message.
						call	write.descriptor						; Tell the admin we experienced a bounds error and need to shut down the
															; server.
